...
export function bestJaccardMatch(
  ...
  const add = (
    ...
    for (const [word, count] of newLine) {
      ...
      if (targetCount > 0) {
        ...
        const newIntersectionCount = Math.min(
  ...
  for (let i = 0; i < Math.min(windowSize, lines.length); i++) {
    for (const [wordInThisLine, wordInThisLineCount] of wordsForEachLine[
      i
    ].entries()) {
      windowWords.set(
        wordInThisLine,
        (windowWords.get(wordInThisLine) || 0) + wordInThisLineCount
      );
    }
  }
...
export function jaccardDistance(
  ...
  if (union < 0) {
    throw new Error(
      "intersection can't be greater than the sum of left and right"
    );
  }
  if (union === 0) {
    return 0;
