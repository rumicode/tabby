...
export class BfgRetriever implements ContextRetriever {
  ...
  constructor(private context: vscode.ExtensionContext) {
    ...
    this.loadedBFG.then(
      ...
      (error) => {
        ...
        console.debug("CodyEngine", "failed to initialize", error);
      },
    );
  ...
  private isWorkspaceIndexed(folder: vscode.Uri): boolean {
    ...
    console.debug("CodyEngine", "Checking if folder is indexed", uri);
  ...
  private async indexEntry(params: { repository?: SimpleRepository; workspace?: vscode.Uri }): Promise<void> {
    ...
    try {
      ...
      const label = repository ? `${repository.uri.fsPath}:${repository.commit}` : workspace ? workspace.fsPath : "";
      if (label) {
        console.debug("CodyEngine", `gitRevision/didChange ${label} indexing time ${elapsed}ms`);
      }
    } catch (error) {
      console.debug("CodyEngine", `indexing error ${error}`);
    }
  }

  public async retrieve({ document, position, docContext, hints }: ContextRetrieverOptions): Promise<ContextSnippet[]> {
    try {
      ...
      const bfg = await this.loadedBFG;
      if (!bfg.isAlive()) {
        console.debug("CodyEngine", "not alive");
        return [];
      }
      ...
      console.debug("CodyEngine", "contextAtPosition", responses);
      ...
      console.debug("CodyEngine:error", `${error}`);
      return [];
    }
  }
  ...
  private loadBFG(): Promise<MessageHandler> {
    ...
    return new Promise<MessageHandler>((resolve, reject) => {
      console.debug("CodyEngine", "loading bfg");
