...
export async function downloadBfg(context: vscode.ExtensionContext): Promise<string | null> {
  ...
  serializeBfgDownload = (async () => {
    ...
    if (userBfgPath) {
      ...
      console.debug("CodyEngine", `using user-provided path: ${userBfgPath} ${bfgStat.isFile()}`);
      return userBfgPath;
    }

    const osArch = getOSArch();
    if (!osArch) {
      console.debug("CodyEngine", "getOSArch returned nothing");
      return null;
    }
    const { platform, arch } = osArch;

    if (!arch) {
      console.debug("CodyEngine", "getOSArch returned undefined arch");
      return null;
    }

    if (!platform) {
      console.debug("CodyEngine", "getOSArch returned undefined platform");
      return null;
    }
    ...
    if (isAlreadyDownloaded) {
      console.debug("CodyEngine", `using downloaded path "${bfgPath}"`);
      return bfgPath;
    }
    ...
    try {
      await vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          console.debug("CodyEngine", bfgPath);
          ...
          console.debug("CodyEngine", `downloaded cody-engine to ${bfgPath}`);
        },
      );
...
async function downloadBfgBinary(url: string, destination: string): Promise<void> {
  console.debug("CodyEngine", `downloading from URL ${url}`);
